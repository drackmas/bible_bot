...
class BiblePageState {
  ...
  const BiblePageState({required this.reference, required this.page});
}

class BiblePaginator {
  static const String prefix = 'bible_page';

  static const String pageIndicatorId = 'bible_page_indicator';
  ...
  static BiblePageState? parseCustomId(String customId) {
    if (!customId.startsWith('$prefix:')) {
      return null;
    }

    try {
      var encoded = customId.substring('$prefix:'.length);
      ...
      final translation = map['v']?.toString();

      final book = map['b']?.toString();

      final chapter = int.tryParse(map['c']?.toString() ?? '');

      final page = int.tryParse(map['p']?.toString() ?? '');
      ...
      final start = map['s'] == null ? null : int.tryParse(map['s'].toString());

      final end = map['e'] == null ? null : int.tryParse(map['e'].toString());
  ...
  static Future<MessageBuilder> buildMessage({
    ...
    final pages = await BibleEmbedRenderer.renderPages(
      book: reference.book,
    ...
    final embed = BibleEmbedRenderer.buildEmbed(
      book: reference.book,
      chapter: reference.chapter,
      translation: reference.translation,
      page: currentPage,
    ...
    final components = <ComponentBuilder<Component>>[];

    if (pages.length > 1) {
      final rowComponents = <ComponentBuilder<Component>>[];

      if (safePage > 0) {
        rowComponents.add(
          ButtonBuilder(
            style: ButtonStyle.secondary,
            customId: createCustomId(reference: reference, page: safePage - 1),
            label: '◀ Previous',
          ),
        );
      }
      ...
      components.add(ActionRowBuilder(components: rowComponents));
    }

    return MessageBuilder(embeds: [embed], components: components);
  }
  ...
  static Future<MessageUpdateBuilder> buildUpdate({
    ...
    final message = await buildMessage(reference: reference, page: page);
